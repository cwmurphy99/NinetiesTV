...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows.Select(s => s.ImdbRating).Average();
            //return shows.Average(s => s.ImdbRating); THIS ALSO WORKS
        }
        ...
        {
            return shows.Where(shows => shows.Name.StartsWith("The")).ToList();
        }

        // 10. Return all shows except for the lowest rated show.
        static List<Show> AllButWorst(List<Show> shows)
        {
            return shows.Select(s => s).OrderBy(shows => shows.ImdbRating).Skip(1).ToList();
        }
        ...
        {
            return shows.Where(s => s.EndYear - s.StartYear > 10 && s.ImdbRating < 8).OrderBy(s => s.Name).First();
        }
        ...
        {
            string allNames = String.Join(", ", shows.Select(s => s.Name).ToArray());
            int ind = allNames.LastIndexOf(",");
            string firstNames = allNames.Substring(0, ind);
            string lastName = allNames.Substring(ind + 1);
        ...
        **************************************************************************************************/
        ...
        public static List<string> DistinctGenres(List<Show> shows)
        {
        ...
        public static List<string> BingeTimePerShow(List<Show> shows)
        {
            return shows.Select(s => $"{s.Name} - {s.EpisodeCount * (s.Genres.Contains("Comedy") ? 22 : 42)}").ToList();
        }
        ...
        public static int HighestAverageIMDBYear(List<Show> shows)
        {
            return shows.SelectMany(s => Enumerable.Range(s.StartYear, s.EndYear - s.StartYear + 1).Select(y => new { Year = y, Show = s }))
            .GroupBy(sg => sg.Year)
            .OrderByDescending(sg => sg.Average(sg => sg.Show.ImdbRating))
            .First().Key;
        }
